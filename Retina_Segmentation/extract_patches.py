...
def get_data_training(DRIVE_train_imgs_original,
    ...
    assert(np.min(train_masks)==0 and np.max(train_masks)==255)
...
def get_data_testing(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width):
    ...
    #check masks are within 0-1
    assert(np.max(test_masks)==255  and np.min(test_masks)==0)
    ...
    for i in range(full_imgs.shape[0]):  #loop over the full images
        ...
        while k <patch_per_img:
            ...
            if i <= 17:
                ...
                Image.fromarray(patch[0]).convert("L").save(f'./image/test/img_{i}_patch_{k}.png')
                Image.fromarray(patch_mask[0]).convert("L").save(f'./mask/test/mask_{i}_patch_{k}.png')
